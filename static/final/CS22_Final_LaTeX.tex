...
{\setcounter{enum22i}{0}
	...
	For all $0 < k < n$ define $S(n,k)$ to be the number of partitions of an $n$-element set into $k$ blocks.
	\begin{22enumerate}
	 \item Argue via counting argument that the following recurrence identity holds for integers $n \geq 2, k \geq 2$:
	$$S(n,k) = kS(n-1,k) + S(n-1,k-1).$$
	
	\item How many surjective functions from an $n$-element set to a $k$-element set are there in terms of $S(n,k)$, $n$, and $k$? Note that here $0 < k \leq n$. Justify your answer.
...
{\setcounter{enum22i}{0}
    ...
    A \textit{bipartite graph} is a graph whose vertices can be partitioned into two disjoint vertex sets, $L$ and $R$, such that every edge has one endpoint in $L$ and the other in $R$. See the figure below for an example.
      ...
      Take two integers $a,b \in \Z^+$ such that $a < b$, and create a bipartite graph where the vertex sets are $L=R=\{n \in Z \;|\; n \in [0, b-1]\}$. Create edges in the following manner: connect with an edge $i \in L$ to both $i \in R$ and $\mathrm{rem}(i+a,b) \in R$. An example is given below for $a=2$, $b=3$.
      ...
      \begin{22enumerate}
          ...
          \item Consider now the graph where we connect edge $i \in L$ to $\mathrm{rem}(i+ka,b) \in R$ for all $k \in \Z$. If we let $a=1$, find the expected distance between two distinct, randomly selected vertices with respect to $b$, justifying your answer. No need to simplify your expression.
...
\textbf{Claim:} For $n\ge 4$, if an $n$ vertex graph has at least $n+1$ edges, it must contain a subgraph that is a triangle (a simple cycle of length $3$).
...
\begin{22enumerate}
    ...
    \hint{Use the actual Pigeonhole Principle!}
