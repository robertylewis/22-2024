...
\textbf{Please Read:} For the purposes of this problem, you may assume the existence of a bijection between paths on a grid and strings of letters/digits that represent the moves that are chosen along the paths. This means that you are allowed to represent paths by strings that represent sequences of moves.
	
For example, let $l$ = left-move and $u$ = up-move. Then the path in a Cartesian grid from $(0,0)$ to $(0,1)$ to $(-1,1)$ can be represented by the string `$ul$'.
	...
	\end{center}
	 ...
	 \item Now consider a generalized Cartesian grid with height and width equal to $n \geq 1$, an $(n+1)$ by $(n+1)$ grid of dots. Here we label the lower left-hand corner dot by $(0,0)$ and the upper right-hand corner dot by $(n,n)$. How many paths of shortest length go from the $(0, 0)$ dot to the $(n, n)$ dot? Explain your reasoning.
